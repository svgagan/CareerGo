...
<html>
<head>
	<title>Career Go | Admin-Company Details</title>
</head>
          ...
          <div class="row">
            ...
            <div class="col-md-12">
              ...
              <div class="box box-success">
                ...
                <!-- form start -->
                <div class="box-body">
                  <table id="company" class="table table dataTable"><!--  table table-bordered table-hover dataTable -->
                    <thead class="no-sort">
                      <tr>
                    ...
                    <tbody id="companyDetails">
                      <c:forEach items="${companyList}" var="companyList">
                      	<tr>
                      		<td style="display:none;">${companyList.id}</td>
							<td>${companyList.companyname}</td>
							<td>${companyList.email}</td>
							<td>${companyList.address}</td>
							<td>${companyList.phone}</td>
							<td>${companyList.sDate}</td>
							<c:if test="${companyList.bStatus == false}">
								<td style="cursor: pointer;" title="click to verify" onclick="updateDetails(${companyList.id});"><span class="label label-warning">${companyList.vStatus}</span></td>
							</c:if>
							<c:if test="${companyList.bStatus == true}">
								<td onclick="editApplicant(${companyList.id});"><span class="label label-success">${companyList.vStatus}</span></td>
							</c:if>
                      	</tr>
                      </c:forEach>
                    </tbody>
